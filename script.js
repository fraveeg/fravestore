// script.js - FULL CODE (Add showNotification global function and sound)
...
const allProducts = [
    // --- CUSTOM FRAMED POSTER (NEW & TOP PRODUCT) ---
    ...
    // Framed Posters (existing products)
...
window.updateBasketDisplay = function() {
    ...
    try {
        ...
        window.basket.forEach((item, i) => {
            ...
            // Add custom design details to basket item display if applicable
            let customText = '';
            if (item.isCustomDesign) {
                // Determine original base price based on category
                // Safely get original price from item data, or default if missing
                const originalBasePrice = item.originalPrice || (item.category === "Framed Poster" ? item.basePrice : item.price);
                const customFee = item.price - originalBasePrice; // Calculate actual custom fee added
            ...
            itemDiv.innerHTML = `
                <div class="basket-item-details">
                    ...
                    <div>
                        ...
                        <p class="basket-item-price">${item.category === "Framed Poster" && item.size ? 'Size: ' + item.size : 'Price: ' + item.price} L.E x ${item.qty} = ${item.total} L.E${customText}</p>
                    </div>
                </div>
...
// (Requires a <div id="notificationContainer"> in your HTML)
                ...
                if (filteredProducts.length === 0) {
                    ...
                    filteredProducts.forEach(product => {
                        ...
                        if (product.category === "Framed Poster") {
                            ...
                            productLink.href = "javascript:void(0)"; // Fallback for removed all_products page
                        }
