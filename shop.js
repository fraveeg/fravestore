// shop.js - Specific to Framed Posters (Updated with Sorting and Debugging Logs)
document.addEventListener('DOMContentLoaded', () => {
    ...
    const applyFiltersBtn = document.getElementById('applyFiltersBtn'); // The button in question
    ...
    if (!container || !productCountSpan) {
        console.error("shop.js: CRITICAL ERROR: Products container or product count span not found. Page might not render correctly.");
        return;
    }
    ...
    function filterProducts() {
        ...
        if (currentSortOrder === 'lowToHigh') {
            ...
            // Default sort: custom product first, then original order
            filtered.sort((a, b) => {
    ...
    if (filterIcon && filterOverlay && filterCloseBtn && priceRangeSlider && applyFiltersBtn && clearFiltersBtn) {
        ...
        filterOverlay.addEventListener('click', (e) => e.stopPropagation()); // Prevent filter menu clicks from closing main menus
