// stickers.js - Full Code (No functional changes needed, as it pulls from global allProducts)
document.addEventListener('DOMContentLoaded', () => {
    console.log("stickers.js: DOMContentLoaded fired. Initializing page elements.");

    // Define initial product data for Stickers (assuming allProducts is global from script.js)
    // This will now include your 10 new sticker collections from script.js
    const initialProductsData = allProducts.filter(p => p.category === "Sticker");
    let currentFilteredProducts = [...initialProductsData]; // Start with all stickers
    ...
    let currentMaxPrice = parseInt(priceRangeSlider ? priceRangeSlider.max : 50); // Max price for stickers (still 50)
    let currentSortOrder = 'default';

    if (!container || !productCountSpan) {
        console.error("stickers.js: CRITICAL ERROR: Products container or product count span not found. Page might not render correctly.");
        return;
    }
    ...
    container.addEventListener('click', e => {
        ...
        if (existingIndex > -1) {
            ...
            basket.push({
                ...
                name: product.name,
                image: product.image,
                originalPrice: product.price, // Store original price for consistency
                price: pricePerUnit,
                qty: qty,
