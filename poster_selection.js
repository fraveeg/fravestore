// poster_selection.js - Handles the selection of individual posters for a pack (allowing multiple selections of same design, with improved visual controls and custom box fix)
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Re-initialize selectedDesigns based on session storage if it exists, otherwise empty array.
    let selectedDesigns = (currentPackSelection && currentPackSelection.selectedDesigns) ?
                          currentPackSelection.selectedDesigns.filter(d => d != null) : [];


    const requiredPosterCount = currentPackSelection.posterCount;
    const CUSTOM_DESIGN_FEE_PER_POSTER = 0; // Define custom fee for individual posters
    ...
    function updateSelectionStatus() {
        ...
        if (selectedDesigns.length === requiredPosterCount) {
            ...
            if (!addPackToBasketBtn.dataset.notifiedMax || addPackToBasketBtn.dataset.notifiedMax === 'false') {
                 window.showNotification(`You have selected ${requiredPosterCount} posters. Ready to add to basket!`);
                 addPackToBasketBtn.dataset.notifiedMax = 'true';
            }
        } else {
            addPackToBasketBtn.disabled = true;
            addPackToBasketBtn.dataset.notifiedMax = 'false';
        }
        ...
        // Update individual product card counts and 'selected' class
        document.querySelectorAll('.product').forEach(productDiv => {
            const posterId = productDiv.querySelector('.add-design-btn') ? productDiv.querySelector('.add-design-btn').getAttribute('data-id') : null;
            if (!posterId) return;

            const customCheckbox = productDiv.querySelector(`#custom-${posterId}`);
            const imageInput = productDiv.querySelector(`#image-${posterId}`);

            // Count how many times this specific poster (by ID, and including its custom state/filename if custom) is in selectedDesigns
            const currentSelectedCount = selectedDesigns.filter(design => {
                let matches = design.id === posterId;
                if (design.isCustomDesign || (customCheckbox && customCheckbox.checked)) {
                    // For custom designs, compare custom state AND file name (if available from UI)
                    const currentFileInputFilename = (imageInput && imageInput.files.length > 0) ? imageInput.files[0].name : 'Image not provided on order, client must send via WhatsApp.';
                    matches = matches && design.isCustomDesign === (customCheckbox && customCheckbox.checked) && design.customImageFileName === currentFileInputFilename;
                }
                return matches;
            }).length;

            const countBubble = productDiv.querySelector('.selected-count-bubble');
            if (countBubble) {
                countBubble.innerText = currentSelectedCount;
                if (currentSelectedCount > 0) {
                    productDiv.classList.add('selected');
                    countBubble.style.transform = 'scale(1)'; // Show the bubble
                } else {
                    productDiv.classList.remove('selected');
                    countBubble.style.transform = 'scale(0)'; // Hide the bubble
                }
            }

            // Update add/remove button disabled states
            const addDesignBtn = productDiv.querySelector(`.add-design-btn[data-id="${posterId}"]`);
            const removeDesignBtn = productDiv.querySelector(`.remove-design-btn[data-id="${posterId}"]`);

            if (addDesignBtn && removeDesignBtn) {
                const isCustomChecked = customCheckbox && customCheckbox.checked;
                const isFileMissing = isCustomChecked && (!imageInput || imageInput.files.length === 0);

                addDesignBtn.disabled = isFileMissing || (selectedDesigns.length >= requiredPosterCount);
                removeDesignBtn.disabled = currentSelectedCount === 0; // Can only remove if at least one is selected
            }
        });
    }
    ...
    function renderIndividualPosters(postersToRender) {
        ...
        postersToRender.forEach((poster) => {
            ...
            // Count how many times this specific poster (by ID) is in selectedDesigns
            const currentSelectedCount = selectedDesigns.filter(design => design.id === poster.id).length;
            // The 'selected' class is now applied by updateSelectionStatus based on actual count > 0
            // It will also handle the bubble visibility and text

            // Custom Design Options HTML
            const customDesignHtml = poster.isCustomizable ? `
                <div class="custom-design-option">
                    <label>
                        <input type="checkbox" id="custom-${poster.id}" class="custom-design-checkbox" ${selectedDesigns.some(d => d.id === poster.id && d.isCustomDesign) ? 'checked' : ''}>
                        Custom Design (+${CUSTOM_DESIGN_FEE_PER_POSTER} L.E)
                    </label>
                    <div class="custom-image-upload" id="upload-section-${poster.id}" style="display:${selectedDesigns.some(d => d.id === poster.id && d.isCustomDesign) ? 'block' : 'none'};">
            ...
            productDiv.innerHTML = `
                ...
                <p class="poster-price" style="display:none;">${poster.price} L.E</p>
                ${customDesignHtml}
                <div class="selection-buttons">
                    <button class="add-design-btn" data-id="${poster.id}" data-name="${poster.name}">Add</button>
                    <button class="remove-design-btn" data-id="${poster.id}" data-name="${poster.name}">Remove</button>
                </div>
                <div class="selected-count-bubble">0</div>
            `;
            ...
            if (poster.isCustomizable) {
                ...
                const addDesignBtn = productDiv.querySelector(`.add-design-btn[data-id="${poster.id}"]`);
                const removeDesignBtn = productDiv.querySelector(`.remove-design-btn[data-id="${poster.id}"]`);

                // Restore custom checkbox state if a custom version of this design was previously selected
                const lastSelectedCustomInstance = selectedDesigns.find(d => d.id === poster.id && d.isCustomDesign);
                if (lastSelectedCustomInstance && customCheckbox) {
                    customCheckbox.checked = true;
                    if (uploadSection) uploadSection.style.display = 'block';
                    // Note: Cannot restore file input for security, but filename is in selectedDesigns
                }
                ...
                if (addDesignBtn && removeDesignBtn) {
                    const updateButtonStates = () => {
                        ...
                        addDesignBtn.disabled = isFileMissing || (selectedDesigns.length >= requiredPosterCount);
                        // A design can only be removed if there's at least one instance selected
                        removeDesignBtn.disabled = selectedDesigns.filter(d => d.id === poster.id).length === 0;
                    };

                    // Initial button states
                    updateButtonStates();

                    // --- IMPORTANT: Stop propagation for clicks on custom elements ---
                    // This is the key fix to allow clicking the checkbox and file input themselves
                    customCheckbox && customCheckbox.addEventListener('click', (e) => {
                        e.stopPropagation(); // Prevent main container listener from firing
                    });
                    imageInput && imageInput.addEventListener('click', (e) => {
                        e.stopPropagation(); // Prevent main container listener from firing
                    });
                    ...
                    customCheckbox && customCheckbox.addEventListener('change', () => {
                        ...
                        updateButtonStates();
                        // We need to re-evaluate how many of this *specific custom/non-custom* design are selected
                        // and then update the main selection status
                        updateSelectionStatus(); // Re-evaluate total and individual counts
                    });
        ...
        updateSelectionStatus(); // Initial status update after rendering
    }
    ...
    // Unified click handler for the product container
    container.addEventListener('click', e => {
        const productDiv = e.target.closest('.product');
        if (!productDiv) return; // Not a click on a product card

        const posterId = productDiv.querySelector('.add-design-btn') ? productDiv.querySelector('.add-design-btn').getAttribute('data-id') : null;
        if (!posterId) return; // Ensure it's a valid product card with buttons
        ...
        // Get current state of custom options from the DOM elements for this product
        const customCheckbox = productDiv.querySelector(`#custom-${posterId}`);
        const imageInput = productDiv.querySelector(`#image-${posterId}`);
        ...
        // Handle explicit "Add" button clicks
        if (e.target.classList.contains('add-design-btn')) {
            // Check if custom is selected but file is missing
            if (isCustomDesign && imageInput && imageInput.files.length === 0) {
                window.showNotification('Please select your custom image file for this design first.', 'warning');
                return;
            }

            // Check if total selections already meet the required count
            if (selectedDesigns.length >= requiredPosterCount) {
                window.showNotification(`You have already selected all ${requiredPosterCount} posters. Click 'Remove' to change a selection.`, 'warning');
                return;
            }

            // Add action: push a new instance of the design
            const fullPosterDetails = allIndividualPosters.find(p => p.id === posterId);
            if (!fullPosterDetails) {
                console.error("Selected poster details not found for ID:", posterId);
                return;
            }
            ...
            window.showNotification(`Added "${posterName}". Total selected: ${selectedDesigns.length}`);

        }
        // Handle explicit "Remove" button clicks
        else if (e.target.classList.contains('remove-design-btn')) {
            // Find and remove only ONE instance of this specific design (matching custom state and filename for precision)
            ...
            if (indexToRemove !== -1) {
                ...
                window.showNotification(`Removed one "${posterName}". Total selected: ${selectedDesigns.length}`);
            } else {
                window.showNotification(`"${posterName}" is not currently selected to remove.`, 'warning');
            }
        }
        // Important: Clicks on custom checkbox or file input should NOT trigger add/remove logic here.
        // Their default behavior should be allowed, and then re-render is triggered by their change listeners.
        else if (e.target.classList.contains('custom-design-checkbox') || e.target.closest('.custom-image-upload')) {
            return; // Exit, let the specific handlers for these elements manage their behavior.
        } else {
             // Clicks on other parts of the product card (image, name, etc.)
             // Currently, these clicks do nothing to add/remove, as we have explicit buttons.
             // If you wanted "click anywhere on card to add", this logic would go here,
             // but it would require careful handling to not conflict with the checkbox/file input.
             return;
        }

        // After any add/remove action, re-render to update counts and button states
        // Instead of a full re-render, updateSelectionStatus() now handles granular updates.
    ...
    // updateSelectionStatus() is called by renderIndividualPosters
    window.updateBasketDisplay();
